...
function successAfterNTrials(n, status) {
    return function (req, res, next) {
        if (typeof trials[req.path] === 'undefined') {
        ...
        console.log(`trials[req.path]: ${trials[req.path]}, n: ${n}`)

        if (trials[req.path] > n) {
            console.warn('ici ok 200')
            return res
                .status(200)
                .end(`Success after ${trials[req.path]} trials.`)
        }
...
export default [
    ...
    /*{
        path: '500-with-success-finaly',
        description: "Internal Server Error",
        cb: successAfterNTrials(2, 500)
    },
    {
        path: '501',
